...
# Take a turn. The function takes three parameters: board, column, player ('X' or 'O').
...
''' The below function is provided by CodeCademy.

count_streaks() loops through every square in the board. For every square, it looks 4 squares out in all eight directions. 
If none of those squares contain the opposite player’s symbol, then we know it’s possible to make a Connect Four in that direction.
We then add the number of times our symbol appears in those 4 squares. Essentially we’re saying “it’s possible to make a Connect Four 
in this direction and we have 1, 2, or 3 out of the four necessary pieces already placed.
'''	
...
# The below function is provided by CodeCademy and is the strategy that we’re going to try to beat.
...
# This is the evaluation function we write in order to beat the strategy CodeCademy has
...
# The below function sets up a game of Connect Four played by two AIs. 
# For each player, we need to fill in the third parameter of their minimax() call.
def two_ai_game():
    my_board = make_board()
